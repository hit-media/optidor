...
$arEditions = array(
	'ru' => array(
		array(
			...
			'LIST' => array(
				"big_business"=>"Бизнес веб-кластер",
				"portal"=>"Веб-кластер",
				"business"=>"Бизнес",
				"expert"=>"Эксперт",
	...
	//			'intranet_company' => 'Компания',
		...
		array(
			...
			'LIST' => array(
				...
				'gov/gossite_ext' => 'Официальный сайт государственной организации (расширенный)',
				'gov/gosportal' => 'Внутренний портал государственной организации',
				'gossite_ua' => 'Офіційний сайт державної організації для України',
			),
		),
		array(
			'NAME' => 'Решения для образования',
			'LIST' => array(
				'edu/eduportal' => 'Внутренний портал учебного заведения',
				'edu/edusite' => 'Сайт учебного заведения',
				'school' => 'Сайт школы',
				'school_base' => 'Сайт школы (базовый)',
				'ppk' => 'Портал повышения квалификации',
			)
		),
		array(
			'NAME' => 'Решения для 1С:Франчайзи',
			'LIST' => array(
		...
		array(
			'NAME' => 'Решения для здравоохранения',
			'LIST' => array(
				'medsite_base' => 'Сайт медицинской организации (Базовый)',
				'medsite' => 'Сайт медицинской организации (Расширенный)',
				'medportal' => 'Портал органа управления здравоохранением',
			)
		),
		array(
...
elseif ($strAction=="LOAD")
{
	...
	{
		$txt = $status;
		$script = "<script>document.location = \"".$this_script_name."?action=LOAD&edition=".urlencode($_REQUEST['edition'])."&url=".urlencode($url)."&lang=".urlencode(LANG)."&LICENSE_KEY=".urlencode($_REQUEST["LICENSE_KEY"])."&action_next=".urlencode($_REQUEST["action_next"])."&xz=".rand(0, 32000)."\";</script>\n";
	}
	else
	{
		$txt = $status;
		$script = "<script>document.location = \"".$this_script_name."?action=UNPACK&by_step=Y&filename=".urlencode(basename($strRequestedUrl))."&lang=".urlencode(LANG)."&xz=".rand(0, 32000)."\";</script>\n";
	}
...
//	$iNumDistrFiles = 8000;
	...
	if ($tres)
	{
		if (!$oArchiver->bFinish)
			$script = "<script>document.location = \"".$this_script_name."?action=UNPACK&filename=".urlencode(basename($oArchiver->_strArchiveName))."&by_step=Y&seek=".$oArchiver->iCurPos."\";</script>\n";
		else // finish
		{
